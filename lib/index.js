...
my.generate = (startValue, valueFactory, count) => {
  ...
  return {
    next() {
      ...
      currentCount++
      isDone = currentValue === my.TERMINATE || count && currentCount > count
      return ret
    },
    [Symbol.iterator]() {
      return this
...
my.partition = (iterable, count, includePartial) => {
  let i = my.iter(iterable)
...
my.Iter = class {
  ...
  takeFrom(fn) {
    this.iterator = my.takeFrom(this.iterator, fn)
    return this
  }

  takeUntil(fn) {
    this.iterator = my.takeUntil(this.iterator, fn)
    return this
  }

  takeWhile(fn) {
    this.iterator = my.takeWhile(this.iterator, fn)
    return this
  }
  ...
  partition(count, includePartial) {
    this.iterator = my.partition(this.iterator, count, includePartial)
    return this
  }
